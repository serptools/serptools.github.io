...
export function transformFileTypeData(raw: FileTypeRawData): FileTypeTemplateData {
  ...
  return {
    ...
    developer: (raw as any).developer_name || (raw as any).developer_org || (raw as any).developer,
    popularity: raw.popularity || ((raw as any).rating && (raw as any).votes ? { rating: (raw as any).rating, votes: (raw as any).votes } : undefined),
    image: {
